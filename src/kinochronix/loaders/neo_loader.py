...
class NeoLoader(TimeSeriesSource):
    ...
    def _find_dataset_root(cls, path: Path) -> Path | None:
        ...
        signatures = {"structure.oebin", "*.xml", "*.continuous", "*.ap.meta", "*.lf.meta", "*.ncs", "*.nix"}
    ...
    def can_open(cls, path: Path) -> float:
        """Return 1.0 if neo can discover an IO class for this path."""
        try:
            io_instance = neo.io.get_io(str(path))
            if io_instance is not None:
                io_class_name = io_instance.__class__.__name__
                if io_class_name in ("AsciiSignalIO", "AsciiImageIO", "AsciiSpikeTrainIO"):
                    return 0.5
                return 1.0
        except Exception:
            pass
            
        # Fallback for folder bundles
        if cls._find_dataset_root(path) is not None:
            return 1.0
        ...
        for seg_idx, segment in enumerate(self._block.segments):
            for asig_idx, asig in enumerate(segment.analogsignals):
                ...
                if isinstance(ch_names, str):
                    ch_names = [ch_names] if asig.shape[1] == 1 else [f"{ch_names}_{i}" for i in range(asig.shape[1])]
                ...
                for ch_idx, ch_name in enumerate(ch_names):
                    ...
                    self._schema_channels.append(ChannelInfo(name=unique_name, unit=unit_str, dtype=str(dtype), rate_hz=sr))
    ...
    def read(self, ch: str, t0: float, t1: float, max_points: int) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
