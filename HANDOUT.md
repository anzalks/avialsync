...
- Video/3D horizontal QSplitter: `_media_splitter` gives the video grid and `Tracking3DPane` a
  ...
  otherwise the AOL rig's `DEFAULT_SKELETON_CHAIN` (toe-to-toe through `head_bar`, skipping parts
...
- Transport UX: the full-width **Data Streams** section is distinct from both plots and the
  seek/transport section with the same splitter handles used for video/plot resizing (drawn end to end, D-106). Its controls row, *below* the lanes (D-126), owns Hide and
  compact status; the video tools moved under the videos (`ui/view_toolbar.py`); busy work remains visible while ordinary messages clear shortly.
...
| `ui/controllers/rig_paths.py` | Where a recording's pieces are, for the marker, calibration and wheel controllers alike: camera names, open videos, the `pose-3d` folder, each camera's 2D pose file, and **the one frame-on-screen authority** (D-114) | `camera_name()`, `open_videos()`, `pose3d_dir()`, `pose_2d_file()`, `frame_at()` |
...
| `ui/controllers/wheel_controller.py` | Add Wheel: select any point 1A–3B or Next Point. Each click re-projects at once and **generates the wheel as a registered background job** (`WheelFitWorker`, one at a time, a click during it fitted when it returns); the Wheels tab says "Generating wheel…" and Done Labelling waits for the latest clicks' fit (rule 3, D-123). Done Labelling is live from the moment both ends of bars 1 and 2 have two real camera clicks each and stays live through bar 3 (D-122). **The wheel is always drawn, however poor**; a poor fit is a warning with Re-place, never hidden (D-123). The notification strip says when it is first generated. Done Labelling is one `SetWheelCommand`. Action availability refreshes when a queued video pane is created | `toggled()`, `on_clicked()`, `select_end()`, `next_end()`, `accept()`, `start_checking()`, `stop_checking()`, `replace()`, `reset()` |
| `ui/controllers/wheel_files.py` | When and where a wheel's file is written and read: from the mutation funnel only; read back as pose sources register, never rolling back a wheel already in the session (D-113) | `persist()`, `adopt()` |
| `ui/controllers/wheel_display.py` | What a wheel draws per frame: bars cached by frame index, encoder at the frame's presentation time through its live `TimeMap`; no file IO and no projection on the paint path (D-113, D-116). A projected mark's click target is eight *displayed* pixels at the pane's zoom (D-120) | `pane_drawing()`, `scene()`, `refresh()`, `sample()`, `encoder_binding()`, `frame_and_time()`, `projected_hit_radius()` |
...
| `ui/controllers/wheel_edits.py` | Changing a placed wheel from the Wheels tab: bar count, units, radius (re-fit from its own clicks in the background; the latest edit wins), encoder direction and ratio, removal. Each is one `SetWheelCommand` (D-113) | `edit_spec()`, `edit_binding()`, `remove()`, `spec_from()` |
...
| `ui/wheel_panel.py` | The Wheels tab's one wheel editor and non-modal review: selected point 1A–3B, click count, Next Point, estimate explanation, fit and Done Labelling. A poor fit leads with "Poor fit" and what to check, then the full report, including the radius the clicks imply (D-123). Each placed wheel's row has **Encoder offset**: the encoder source's own offset from its Sources row, applied through `wheel_edits.edit_encoder_offset` (D-124) | `WheelPanel`, `PlacementView`, `describe_fit()` |
| `ui/wheel_tab.py` | Scrollable Wheels inspector tab. Its Add Wheel button is an `ActionButton` on the Edit menu's action; its **Wheel model** and **Wheel bars out of sight** check boxes are `ActionCheckBox`es on View → Overlays' own actions (rules 13, 15, D-124) | `WheelTab`, `install_add_action()`, `install_overlay_actions()` |
| `ui/view_toolbar.py` | The row under the video panes, beside (not under) the 3D view: Flag Frame, Fix Tracker, Add 3D Marker, Add Wheel… (ActionButtons on their menu actions), Snapshot, Fit All Videos, Fullscreen Toggle (D-126) | `ViewToolbar`, `install_fix_tracker_action()`, `install_add_marker_action()`, `install_add_wheel_action()`, `install_fit_videos_action()` |
| `ui/wheel_dialogs.py` | The Add Wheel setup dialog; current recording's `SessionLayout.rotary` takes precedence over reusable bar count, units and radius from declared QSettings. The user chooses once in the dialog whether to remember or update defaults, and can forget them without deleting wheel files | `ask_wheel_setup()`, `WheelSetup` |
...
| `ui/video_grid.py` | N VideoPanes; persistent visibility; single `QGridLayout`; `_relayout()`. `reset_all_views()` is View → Fit All Videos (Ctrl+Shift+0), also a video tools button: every pane to 1.00×, no pan. Fix Tracker and marker/wheel placement are the grid's modes, so a pane built later joins them (D-099, D-112, D-113) | `add_pane()`, `remove_pane()`, `set_pane_visible()`, `visible_panes()`, `set_grid_mode()`, `set_point_edit_mode()`, `set_point_edits()`, `set_marker_place_mode()`, `set_custom_markers()`, `set_reprojection_source()`, `set_wheel_source()`, `custom_marker_at()`; signals `marker_clicked`, `custom_point_moved` |
| `ui/plot_pane.py` | Coordinator for linked pyramid plot rows, presentation, shared X/Y state, and navigator signal. The row stack lives in a `QScrollArea` (`_plot_scroll`): pyqtgraph pins its scene rect to the viewport, so a scrollbar on the graphics view itself can never have a range. The plot controls row and the time span sit below the rows (D-126) | `load_channels()`, `set_window_duration()`, `set_cursor()`, `set_channel_y_mode()` |
...
| `WheelStore.observe` | `MainWindow._on_wheels_changed` → `wheel_display.refresh` (views + Wheels tab); files are written by `WindowMutationTarget.set_wheel`, never by the observer |
...
loaded when wheels are adopted or placed, never while painting. Wheel files are `.wheel.toml`, which
no loader claims, but `is_wheel_path()` is consulted beside the marker and correction predicates in
`LoaderRegistry.find_best_loader` and the drop scan all the same.
