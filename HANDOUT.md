...
| `ui/controllers/wheel_controller.py` | Add Wheel end to end: dialog → calibration → guided clicks → live fit → Accept (`SetWheelCommand`) → Verify; re-fit, re-place, remove (D-113) | `toggled()`, `on_clicked()`, `accept()`, `start_checking()`, `edit_spec()`, `edit_binding()`, `remove()`, `replace()`, `reset()` |
...
| `ui/transport.py` | Seek row with playhead/A-B/rate controls + D-027 named, conditional Data Streams header/status. Owns the displayed time mode and epoch: anything writing a master time in the same words calls `format_master_time()` rather than keeping a second copy (D-020, D-101). The Data Streams header carries Fix Tracker and Add 3D Marker as buttons driven by their QActions (rule 15, D-112) | `set_time()`, `set_bounds()`, `format_master_time()`, `status_text()`, `set_source_coverage(source_id, t0, t1, kind, group="")` — a non-empty `group` merges the span into one shared lane (D-083); an empty span at the origin removes it — `set_ttl_events()`, `set_gap_events()`, `set_message_events()`, `set_annotation_markers()`, `set_status()`, `install_fix_tracker_action()`, `install_add_marker_action()` |
