...
conda run -n avialsync python tools/generate_session_screenshot.py <folder>  # your own recording
```
...
`docs/_static/screenshots/session_overview.gif` and reads the session.
...
in its filename. That default is the point — the animation used to *require* a real recording
folder, so the one image at the top of the README could only be rebuilt by whoever had the field
data on their machine, and AGENTS.md rule 5 is explicit that tooling must not assume the user's
private data is available.

Its camera is drawn by `avialsync.demo`, not copied from `tests/fixtures/videos/`. Those are
frame-index test patterns for the golden sync tests: near-black by design, with the frame number in
